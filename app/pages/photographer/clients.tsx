...
export default function Clients({ userType = 'photographer' }: ClientsProps) {
  ...
  // Calculate total tips from all clients (tips are from viewers/clients)
  const totalTips = 275; // Consistent tips amount across all pages

  // Calculate total completed events for bonus calculation
  const totalCompletedEvents = clients.reduce((sum, c) => {
    return sum + c.bookings.filter(b => b.status === 'completed').length;
  }, 0);

  // Platform bonus: consistent amount across all pages
  const bonusEligibleSets = Math.floor(totalCompletedEvents / 10);
  ...
  // Total balance = consistent amount across all pages
  const totalBalance = 4975; // Consistent balance amount across all pages
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', backgroundColor: '#F9FAFB' }}>
      ...
      <div style={{
        ...
        <Topbar tipsAmount={totalTips} bonusAmount={platformBonus} balanceAmount={totalBalance} userRole={userType} />
        ...
        <div style={{ flex: 1, overflowY: 'auto', backgroundColor: '#F9FAFB', paddingLeft: '1.25rem', paddingRight: '1.25rem', paddingTop: '1rem', paddingBottom: '1rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '1rem', marginBottom: '1.5rem' }}>
            ...
            <div style={{ backgroundColor: totalTips > 0 ? '#FFFBEB' : 'white', borderRadius: '0.75rem', padding: '1rem', border: totalTips > 0 ? '1px solid #FCD34D' : '1px solid #E5E7EB' }}>
              <div style={{ fontSize: '0.85rem', color: totalTips > 0 ? '#92400E' : '#6B7280', marginBottom: '0.25rem', display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
                <GiftIcon />
                Tips
              </div>
              <div style={{ fontSize: '1.5rem', fontWeight: '700', color: totalTips > 0 ? '#F59E0B' : '#6B7280' }}>
                ${totalTips.toLocaleString()}
              </div>
              <div style={{ fontSize: '0.7rem', color: '#9CA3AF', marginTop: '0.25rem' }}>From clients</div>
            </div>
            <div style={{ backgroundColor: platformBonus > 0 ? '#F0FDF4' : 'white', borderRadius: '0.75rem', padding: '1rem', border: platformBonus > 0 ? '1px solid #BBF7D0' : '1px solid #E5E7EB' }}>
              <div style={{ fontSize: '0.85rem', color: platformBonus > 0 ? '#166534' : '#6B7280', marginBottom: '0.25rem', display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
                <GiftIcon />
                Bonus
              </div>
              <div style={{ fontSize: '1.5rem', fontWeight: '700', color: platformBonus > 0 ? '#16A34A' : '#6B7280' }}>
                ${platformBonus.toLocaleString()}
              </div>
              <div style={{ fontSize: '0.7rem', color: '#9CA3AF', marginTop: '0.25rem' }}>{totalCompletedEvents}/10 events ({bonusEligibleSets > 0 ? '10%' : '0%'})</div>
            </div>
            <div style={{ backgroundColor: 'white', borderRadius: '0.75rem', padding: '1rem', border: '1px solid #E5E7EB' }}>
                ...
                >
                  ...
                  {client.review ? (
                    ...
                    <div style={{ display: 'flex', gap: '0.375rem' }}>
                      <Link
                        href={`/user/photographer/inbox?client=${encodeURIComponent(`${client.firstName} ${client.lastName}`)}`}
                        onClick={(e) => e.stopPropagation()}
                        style={{
                          flex: 1,
                          padding: '0.375rem',
                      ...
                      <Link
                        href="/user/photographer/gallery"
                        onClick={(e) => e.stopPropagation()}
                        style={{
                          flex: 1,
                          padding: '0.375rem',
                    ...
                    >
                      <GiftIcon />
                      Tips: {client.tipsAndBonuses.filter(t => t.type === 'tip').length > 0
                        ? `$${client.tipsAndBonuses.filter(t => t.type === 'tip').reduce((sum, t) => sum + t.amount, 0)}`
                        : 'None'
                      }
                    </button>
                </div>
        ...
        {selectedClient && (
          <div>
            ...
            <div style={{ display: 'flex', gap: '0.75rem' }}>
              <Link
                href={`/user/photographer/inbox?client=${encodeURIComponent(`${selectedClient.firstName} ${selectedClient.lastName}`)}`}
                style={{
                  flex: 1,
                  padding: '0.75rem',
                  borderRadius: '0.5rem',
        ...
        {tipsClient && (
          <div>
            ...
            <div style={{
              ...
              <div style={{ fontSize: '0.8rem', color: '#92400E' }}>Total Tips Received</div>
              <div style={{ fontSize: '0.75rem', color: '#6B7280', marginTop: '0.25rem' }}>
                {tipsClient.tipsAndBonuses.filter(t => t.type === 'tip').length} tip{tipsClient.tipsAndBonuses.filter(t => t.type === 'tip').length !== 1 ? 's' : ''} from this client
              </div>
            </div>
            ...
            <div>
              ...
              {tipsClient.tipsAndBonuses.filter(t => t.type === 'tip').length > 0 ? (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                  {tipsClient.tipsAndBonuses.filter(t => t.type === 'tip').map((item) => (
